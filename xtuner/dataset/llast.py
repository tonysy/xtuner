...
from xtuner.utils import LLAST_AUDIO_TOKEN_INDEX, IGNORE_INDEX
...
class LLaSTDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 cv_dir=None,
                 with_asr=True,
                 en2x_sample_step=1, # sample step of en2x data (1 means, no sampling, 10 menas sampling 1/10 data)
                 check_audio_path=False,
                 x2en=["fr", "es"],
                 en2x=["ja", 'de']):
    ...
    def get_data_ids(self,
        ...
        for src in x2en:
            data_path = os.path.join(tsv_dir, f'covost_v2.{src}_en.{split}.tsv')
            ...
            if split == 'train' and with_asr:
                ...
                print_log('split is train, augment data with asr data', logger='current')
                raw_data_asr, ids_asr = convert_data(data_path, src_lang=SIM_LANG_DICT[src], mode='asr')
                raw_data += raw_data_asr
                ids += ids_asr
        
        for tgt in en2x:
            data_path = os.path.join(tsv_dir, f'covost_v2.en_{tgt}.{split}.tsv')
            ...
            en2x_raw_data, en2x_ids = en2x_raw_data[::en2x_sample_step], en2x_ids[::en2x_sample_step]
            print_log(f"after sampled, en to {tgt}:{len(en2x_raw_data)}", logger='current')
        ...
        if split == 'train' and with_asr and len(en2x) > 0:
            print_log('split is train, augment data with asr data', logger='current')
            raw_data_asr, ids_asr = convert_data(data_path, src_lang=SIM_LANG_DICT['en'], mode='asr')
            raw_data += raw_data_asr
            ids += ids_asr
    ...
    def get_whisper_features(self, cv_dir, ids, postfix='.mp3.wav'):
        ...
        for item_id in tqdm(ids):
            ...
            wav_path = os.path.join(cv_dir, self.audio_folder, item_id + postfix)
    ...
    def __getitem__(self, index):
        ...
        wav_path = os.path.join(self.cv_dir, data_dict['src_lang'], self.audio_folder,
