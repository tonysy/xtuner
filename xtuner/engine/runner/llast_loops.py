    ...
    def run_iter(self, idx, data_batch: Sequence[dict]) -> None:
        ...
        if self.fp16:
            if is_model_wrapper(self.runner.model):
                self.runner.model.module = self.runner.model.module.to(torch.float16)
            else:
        ...
        with autocast(enabled=self.fp16, dtype=torch.float16):
            ...
            mm_inputs['inputs_embeds'] = mm_inputs['inputs_embeds'].to(llm_data_dtype)
            ...
            if self.end_str:
                generations = [item.split(self.end_str)[0] for item in generations]
